   var context_w = 714,
        context_h = 420,
        map_w = context_w * 0.4379,  // layout for map
    ...
    var margin_p = {
      top: 0, right: 5, bottom: 0, left: 0}
        ...
        width_p = 217 - margin_p.left - margin_p.right,
        height_p = 115 - margin_p.top - margin_p.bottom,
        population_w = 202,
        population_h = 115;
    
    
    var margin_t = {
      top: 5, right: 5, bottom: 5, left: 35}
        ,  // layout for trends
        width_t = 350 - margin_t.left - margin_t.right,
        height_t = 90 - margin_t.top - margin_t.bottom,
        sparkline_w = 340,
        sparkline_h = 90;
  ...
  function get_ranking(scope, category, id){ //categrory ["foster_care_trend" or "population_fast_fact"]; id is the "id" inside that category
    ...
    var counties = data.map(function(d){
      if(category == "foster_care_trend"){
        ...
        if(element.length != 0)
          return {"id": d.id, "value": element[element.length - 1].value};
        else
          return {"id": d.id, "value": NaN};
      }else{
        var element =  (d[category][index]["statistic"]);
        return {"id": d.id, "value": element};
      }
    });
